...
UnitElementFrame::UnitElementFrame(QWidget *parent)
    : ElementFrame(parent) {
    setFixedSize(104, 64);
...
UnitElementFrame::~UnitElementFrame() = default;

void UnitElementFrame::loadData()
{
...
void UnitElementFrame::onIconButtonClicked()
{
    if (m_moreMenu) {
        m_moreMenu->setVisible(!m_moreMenu->isVisible());
        if (m_moreMenu->isVisible()) {
            // menu左上角与UnitElementFrame右上角对齐
            m_moreMenu->move(mapToGlobal(QPoint(width(), 0)));
        }
    }
}

void UnitElementFrame::setIconButtonVisible(bool visible)
{
    if (m_iconButton) {
...
QLayout *UnitElementFrame::getMoreMenuContentLayout() const
{
    return m_moreMenu->getContentLayout();
}

void UnitElementFrame::setupUI()
{
    // 创建垂直布局
